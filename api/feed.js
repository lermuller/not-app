...
function extractText(field) {
  ...
  if (field.__cdata) return field.__cdata.trim()
  if (field._text) return String(field._text).trim()
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const results = items.slice(0, parseInt(count)).map(item => ({
      ...
      description: stripHtml(extractText(item.description || item.summary || item.content)).slice(0, 160),
